...
class clienteApp(QtWidgets.QMainWindow, frmcliente.Ui_frmcliente):
    conexion = conexion()
    caller = None
    buscar="'%%'"
    cod=0
    ...
    def closeEvent(self, *args, **kwargs):
        ...
        if self.menu != 1:
            self.caller.txtcodcliente.setFocus()
    ...
    def devolver(self):
        if self.menu == 0:
            self.caller.setEnabled(True)
            self.caller.txtcodcliente.setText((str(self.txtcod.text())))
            self.caller.consultacliente()
            self.close()
    ...
    def consultar(self):
        ...
        cur = con.cursor()
        cur.execute('select count(*) from cliente where nomcliente like' + self.buscar)
        rows = cur.fetchone()
        ...
        if (int(rows[0]) > 0):
            ...
            self.config(rows)
            cur.execute(
                'select codcliente, nomcliente, telefone, rg, cpf from cliente where nomcliente like' + self.buscar + ' order by codcliente asc')
    ...
    def config(self,rows):
        lista = 'Codigo', 'Cliente', 'Telefone', 'RG', 'CPF'
    ...
    def eliminar(self):
        ...
        con = self.conexion.conectar()
        cur = con.cursor()
        cur.execute('select count(*) from servicio where codcliente=%s',[cod])
        rows = cur.fetchone()
        if int(rows[0])==0:
            ...
            if (mensaje == QtGui.QMessageBox.Yes):
                con = self.conexion.conectar()
                cur = con.cursor()
                cur.execute('delete from cliente where codcliente = %s', [cod])
                con.commit()
                self.cancelar()
                self.consultar()
    ...
    def guardar(self):
        ...
        if (len(nom) > 1):
            ...
            if self.bnd == 0:
                cur.execute(
                    'insert into cliente (nomcliente, telefone, rg, cpf) values(%s,%s,%s,%s)', [nom, tel, rg, cpf])
                con.commit()
                self.cancelar()
                self.consultar()
                ...
                cur.execute('update cliente set nomcliente=%s, telefone=%s, rg=%s, cpf=%s where codcliente=%s', [nom, tel, rg, cpf, cod])
                con.commit()
                self.cancelar()
                self.consultar()
        else:
